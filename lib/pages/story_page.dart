...
import 'package:wgp_video_h5app/models/follows_view.dart';
...
import '../models/recommend_view.dart';
...
class _StoryPageState extends State<StoryPage> with TickerProviderStateMixin {
  ...
  final recommendsSource = RecommendView();
  final followsSource = FollowsView();
  ...
  late PageController followsPageController;
  int followsActualScreen = 0;
  @override
  void initState() {
    ...
    followsPageController = PageController(
      initialPage: followsActualScreen,
      viewportFraction: 1,
    );
    super.initState();
    Get.find<UserProvider>().getCurrentUser().then((value) => {user = value});
    cachedChannel = _homeController.cachedChannels[3] ?? [];
    tabs.addAll(cachedChannel.map((e) => Tab(text: e.name)));
    tabs.add(const Tab(
      text: '關注',
    ));
    tabs.add(const Tab(
  ...
  void dispose() {
    ...
    followsSource.videoSource!.listVideos.forEach((element) {
      if (element.controller != null) {
        element.controller!.dispose();
      }
    });
    followsSource.videoSource!.listVideos.clear();
    suppliers.clear();
    upPages = 0;
    currentLoad = 0;
    upTotal = 0;
  ...
  void _handleTabChanged() {
    ...
    } else if (isFollowsPage()) {
      if (followsPageController.hasClients) {
        followsPageController.jumpToPage(followsActualScreen);
        followsSource.changeVideo(followsActualScreen);
        setState(() {
          isPlay = false;
        });
      }
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      ...
      child: Scaffold(
        ...
        backgroundColor:
            _currentIndex == tabs.length - 1 || _currentIndex == tabs.length - 2
                ? Colors.black
                : Colors.white,
        body: Stack(
          children: [
            NestedScrollView(
                ...
                return [
                  ...
                  SliverPersistentHeader(
                    ...
                    delegate: VDStickyTabBarDelegate1(
                      backgroundColor: _currentIndex == tabs.length - 1 ||
                      ...
                      child: TabBar(
                          ...
                          indicator: BoxDecoration(
                            color: _currentIndex == tabs.length - 1 ||
                                    _currentIndex == tabs.length - 2
                                ? Colors.black
                                : Colors.white,
                          ),
                          labelColor: _currentIndex == tabs.length - 1 ||
                                  _currentIndex == tabs.length - 2
                              ? Colors.white
                              : Colors.black,
              ...
              body: TabBarView(
                  ...
                  children: cachedChannel.map((e) => channelTab(e)).toList() +
                      [videoTab(followsSource, followsPageController), videoTab(recommendsSource, recommendPageController)]),
            )
          ],
        ),
...
//        ),
    ...
    List<Vod> block1Vods =
        showBlock1 && _homeController.cachedBlockVods[block1.id] != null
            ? _homeController.cachedBlockVods[block1.id]!.vods.isNotEmpty
                ...
                : []
            : [];
    List<Widget> leftw = [const SizedBox(width: 8,)];
    return Column(
      children: [
        showBlock1
                ...
                child: Theme(
                  ...
                  child: Scrollbar(
                      ...
                      child: ListView(
                          ...
                          children:
                          leftw + block1Vods.map((e) => buildCard2(e, block1)).toList() + [buildCard1(block1Vods[0], block1)])) ,
                ),
              )
            : Container(),
      ],
  ...
  areaToRecommends(int videoId, int blockId) {
    Get.find<BlockProvider>().getShortVideoPopular(blockId, videoId).then((value) => {
    ...
    return GestureDetector(
      onTap: () async {
        if (data.url == null) {
          ...
          if (data.url.toString().startsWith('http://') ||
              data.url.toString().startsWith('https://')) {
            launch(data.url.toString(),
                webOnlyWindowName: '_blank');
          } else {
            gto(data.url.toString());
          }
        }
              ...
              return Stack(
                children: [
                  supplier != null
                      ...
                      : Container(),
                  supplier == null || supplier.isFollow == null || !supplier.isFollow! ?  Positioned(
                    child: Image(
                      width: 22,
                      height: 22,
