...
abstract class BasePostInfinityScrollController extends GetxController {
  ...
  BasePostInfinityScrollController(
  ...
  Future<InfinityPosts> fetchData(int page);
  ...
  Future<void> _fetchData({bool refresh = false}) async {
    if (!hasMoreData.value && postList.isNotEmpty) return;
    ...
    InfinityPosts newData = await fetchData(nextPage);
    List<Post> newPosts = newData.posts;

    if (newPosts.isNotEmpty) {
      if (refresh) {
        postList.value = newPosts; // 替換現有的列表
      } else {
        postList.addAll(newPosts); // 加入到現有的列表
      }
      page.value = nextPage;
      totalCount.value = postList.length;
